...
const Step5 = () => {
  ...
  const renderCheckboxGroup = (key: keyof typeof formData, options: string[], label: string) => {
    return (
      <View style={styles.formGroup}>
        <Text style={styles.label}>{label}</Text>
        {options.map(option => (
          <View key={option} style={styles.checkboxItem}>
            <TouchableOpacity
              style={[
                styles.checkbox,
                ((formData[key] as string[]) || []).includes(option) && styles.checkboxSelected
              ]}
              onPress={() => handleCheckbox(key, option)}
            >
              <Text style={[
                styles.checkboxText,
                ((formData[key] as string[]) || []).includes(option) && styles.checkboxTextSelected
              ]}>
                ✓
              </Text>
            </TouchableOpacity>
  ...
  return (
    <View>
      ...
      {formData.hydroxyurea === 'Non' && (
        <View style={styles.formGroup}>
          <Text style={styles.label}>Raisons de non-utilisation de l'hydroxyurée</Text>
          <Picker
            selectedValue={formData.hydroxyurea_reasons}
            onValueChange={value => updateFormData('hydroxyurea_reasons', value)}
            style={styles.picker}
          >
            <Picker.Item label="--Sélectionner--" value="" />
            <Picker.Item label="Non disponible en pharmacie" value="Non disponible en pharmacie" />
            <Picker.Item label="Manque de moyens financiers" value="Manque de moyens financiers" />
            <Picker.Item label="Crainte des effets secondaires" value="Crainte des effets secondaires" />
            <Picker.Item label="Crainte de l'irrégularité dans l'approvisionnement du médicament" value="Crainte de l'irrégularité dans l'approvisionnement du médicament" />
            <Picker.Item label="Manque de conviction sur l'intérêt du médicament" value="Manque de conviction sur l'intérêt du médicament" />
            <Picker.Item label="Autre raison" value="Autre raison" />
          </Picker>
        </View>
      )}

      {renderCheckboxGroup('observance', [
        'Mois 1',
        'Mois 2',
        'Mois 3',
      ], 'Observance')}
